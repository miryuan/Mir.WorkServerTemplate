using Mir.WindowsServiceTemplate.Extension;
...
namespace Mir.WindowsServiceTemplate.DependencyInjection.Redis
{
    public class RedisDbContext : IRedisDbContext
    {
        ...
        public RedisDbContext(RedisConnection config)
        {
            CustomKey = config.InstanceName;//前缀
            _conn = ConnectionMultiplexer.Connect(config.Connection);
            //注册事件
            _conn.ConnectionFailed += Connect_ConnectionFailed;
            _conn.ConnectionRestored += Connect_ConnectionRestored;
            _conn.ErrorMessage += Connect_ErrorMessage; ;
            _conn.ConfigurationChanged += Connect_ConfigurationChanged;
            _conn.HashSlotMoved += Connect_HashSlotMoved;
            _conn.InternalError += Connect_InternalError;
        }

        /// <summary>
        /// 选择Redis数据库
        /// </summary>
        /// <param name="num">数据库号</param>
        /// <returns></returns>
        public RedisDbContext ToDb(int num)
        {
            DbNum = num;
            return this;
        }
        ...
        /// 获取一个全新的Redis实例
        /// </summary>
        /// <param name="connectionString">Redis字符串</param>
        /// <returns></returns>
        public ConnectionMultiplexer GetManager(string connectionString = null)
        {
        ...
        /// <returns></returns>
        public bool Set(string key, string value, TimeSpan? expiry = default)
        {
        ...
        /// <returns></returns>
        public bool Set(string key, string value, DateTime expiry = default)
        {
            key = AddSysCustomKey(key);
            if (expiry == default)
            {
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool Set<T>(string key, T obj, DateTime expiry = default)
        {
            key = AddSysCustomKey(key);
            string json = ConvertJson(obj);
            if (expiry == default)
        ...
        /// <param name="expiry">过期时间</param>
        /// <returns></returns>
        public async Task<bool> SetAsync(string key, string value, TimeSpan? expiry = default)
        {
            key = AddSysCustomKey(key);
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool KeyExpire(string key, DateTime expiry = default)
        {
            key = AddSysCustomKey(key);
            return Do(db => db.KeyExpire(key, expiry));
        }
